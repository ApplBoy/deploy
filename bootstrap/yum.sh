...
grep -q '"ghcr.io"' ~/.docker/config.json 2>/dev/null&&{ printf "${B}Already logged in. Re-login? [y/N] ${N}";read d;[[ $d =~ ^[Yy]$ ]]&&docker logout ghcr.io; }||true
...
type -p fzf||{ sudo yum install -y fzf; }
[[ $nginx == 1 ]]&&type -p nginx||{ sudo yum -y install certbot python3-certbot-nginx nginx policycoreutils-python-utils&&sudo systemctl enable --now nginx; }&&for p in 8080 8832;do sudo semanage port -a -t http_port_t -p tcp "$p"||sudo semanage port -m -t http_port_t -p tcp "$p";done&&sudo setsebool -P httpd_can_network_connect 1
